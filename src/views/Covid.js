...
const Covid = () => {
    ...
    let { data: dataCovid, isLoading, isError } = useFetch(`https://api.covid19api.com/country/vietnam?` +
        `from=${moment(today2).format(`YYYY-MM-DD`)}T00%3A00%3A00Z&to=${moment().format(`YYYY-MM-DD`)}T00%3A00%3A00Z`, true)
    return (
        <>
            <table>
                <thead>
